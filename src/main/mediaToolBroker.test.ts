...
describe('media tool broker', () => {
  it('publishes bounded top-level asset and generation function tools', () => {
    ...
    expect(MEDIA_DYNAMIC_TOOLS.map((tool) => tool.name)).toEqual([
      ...
      'bobo_asset_plan',
      'bobo_asset_register',
      'bobo_audio_synthesize',
      'bobo_image_generate',
      'bobo_audio_generate',
      'bobo_model3d_generate',
    ]);
    ...
    const audioTool = MEDIA_DYNAMIC_TOOLS.find((tool) => tool.name === 'bobo_audio_generate');
    ...
    const model3dTool = MEDIA_DYNAMIC_TOOLS.find((tool) => tool.name === 'bobo_model3d_generate');
    ...
    const registerTool = MEDIA_DYNAMIC_TOOLS.find((tool) => tool.name === 'bobo_asset_register');
  ...
  it.each(['bobo_asset_list', 'noobi_asset_list'])('lists only compact public asset fields via %s', async (toolName) => {
    ...
    broker.handle(toolRequest(3, toolName, { kind: 'image', limit: 100 }));
    await vi.waitFor(() => expect(responses).toHaveLength(1));
  ...
  it('creates and reads an expected asset through bobo_asset_plan', async () => {
    const responses: Array<{ id: string | number; result: unknown }> = [];
    ...
    broker.handle(toolRequest(31, 'bobo_asset_plan', {
    ...
    broker.handle(toolRequest(32, 'bobo_asset_plan', { planId: 'hero_portrait' }));
    ...
    broker.handle(toolRequest(33, 'bobo_asset_register', {
      planId: 'hero_portrait',
  ...
  it('creates and registers a short procedural WAV through a dynamic call', async () => {
    ...
    const request = toolRequest(4, 'bobo_audio_synthesize', {
      name: 'game_pickup',
      preset: 'pickup',
      durationSeconds: 0.1,
      seed: 99,
  ...
  it('rejects traversal and invalid names without exposing absolute paths', async () => {
    ...
    broker.handle(toolRequest(5, 'bobo_asset_register', {
      relativePath: 'public/assets/../../outside.wav',
    }));
    broker.handle(toolRequest(6, 'bobo_audio_synthesize', {
      name: '../outside',
      preset: 'hit',
    }));
    await vi.waitFor(() => expect(responses).toHaveLength(2));
  ...
  it('records bounded visual-role metadata for a card-art atlas', async () => {
    ...
    broker.handle(toolRequest(61, 'bobo_asset_register', {
  ...
  it('returns an explicit Codex ImageGen fallback without placing binary data in JSON-RPC', async () => {
    ...
    broker.handle(toolRequest(7, 'bobo_image_generate', {
      name: 'fox_run',
      prompt: 'Four-frame fox run sprite sheet',
      width: 1024,
      height: 1024,
  ...
  it('routes audio and 3D API generation and only returns compact public asset fields', async () => {
    ...
    broker.handle(toolRequest(8, 'bobo_model3d_generate', {
      name: 'player_ship',
  ...
  it('returns a registered Three.js GLB through the same 3D tool when no provider exists', async () => {
    ...
    broker.handle(toolRequest(9, 'bobo_model3d_generate', {
      name: 'zombie_runner',
  ...
  it('passes explicit MiniMax audio purpose, instrumental mode, and lyrics as provider options', async () => {
    ...
    broker.handle(toolRequest(10, 'bobo_audio_generate', {
      name: 'title_music',
      prompt: 'Tense survival-horror title music',
      purpose: 'music',
      instrumental: false,
  ...
  it('rejects unsupported audio purposes before invoking a provider', async () => {
    ...
    broker.handle(toolRequest(11, 'bobo_audio_generate', {
      name: 'rifle_shot',
  ...
  it('rejects a missing audio purpose before invoking a provider', async () => {
    ...
    broker.handle(toolRequest(111, 'bobo_audio_generate', {
      name: 'mystery_audio',
  ...
  it('returns an actionable procedural fallback for generic SFX that MiniMax does not support', async () => {
    ...
    broker.handle(toolRequest(12, 'bobo_audio_generate', {
      name: 'rifle_shot',
      prompt: 'Close-range rifle shot with concrete echo',
      purpose: 'sfx',
    }));
  ...
  it('rejects incomplete image dimensions before calling a provider', async () => {
    ...
    broker.handle(toolRequest(9, 'bobo_image_generate', {
      name: 'hero',
      prompt: 'Hero',
      width: 1024,
    }));
  ...
  it('keeps a safe retryable placeholder when provider generation fails', async () => {
    ...
    broker.handle(toolRequest(90, 'bobo_audio_generate', {
      name: 'battle_theme',
  ...
  it('surfaces only trusted redacted media failures and keeps unexpected errors opaque', async () => {
    ...
    safeBroker.handle(toolRequest(91, 'bobo_audio_generate', {
      name: 'safe_error',
    ...
    opaqueBroker.handle(toolRequest(92, 'bobo_audio_generate', {
      name: 'opaque_error',
      prompt: 'Minimal music test',
      purpose: 'music',
    }));
  ...
  it('passes a real MiniMax business failure through service and broker without provider secrets', async () => {
    ...
    broker.handle(toolRequest(93, 'bobo_audio_generate', {
